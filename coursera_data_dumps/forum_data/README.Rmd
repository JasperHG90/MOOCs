...
You can use the "dbQA" ("database Quick Access") function to get all table names residing in a [SQLite](https://www.sqlite.org/) database, e.g.:
...
One interesting observation to make here is that, while the number of comments and the number of thread posts follow the same pattern (correlation = 0.8), a high number of threads initiated does not seem to drive the number of comments. It's likely that these are influenced by e.g. end-of-week quizzes and other assignments.